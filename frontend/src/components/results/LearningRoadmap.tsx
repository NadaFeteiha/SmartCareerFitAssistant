...
export default function LearningRoadmap({ items }: Props) {
  if (items.length === 0) {
    return (
      <p className="text-slate-500 text-sm text-center py-10">No learning items generated.</p>
    )
  }
      ...
      {sorted.map((item, i) => (
        <div key={i} className="p-5 rounded-xl bg-slate-900 border border-slate-800 space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-semibold text-slate-200 text-sm">{item.skill}</h4>
          </div>
          <p className="text-slate-400 text-sm leading-relaxed">{item.reason}</p>
          <div className="flex items-start gap-2 mt-1 pt-3 border-t border-slate-800">
